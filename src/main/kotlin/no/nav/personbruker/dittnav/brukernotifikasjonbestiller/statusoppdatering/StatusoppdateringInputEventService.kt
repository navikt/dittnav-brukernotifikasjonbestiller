...
class StatusoppdateringInputEventService(
...
) : EventBatchProcessorService<NokkelInput, StatusoppdateringInput> {

    private val log: Logger = LoggerFactory.getLogger(StatusoppdateringInputEventService::class.java)

    override suspend fun processEvents(events: ConsumerRecords<NokkelInput, StatusoppdateringInput>) {
        ...
        metricsCollector.recordMetrics(eventType = Eventtype.STATUSOPPDATERING) {
            events.forEach { event ->
                try {
                    ...
                    val statusoppdateringExternal = event.value()
                    val internalNokkelStatusoppdatering = StatusoppdateringInputTransformer.toInternal(nokkelExternal, statusoppdateringExternal)
                    successfullyValidatedEvents.add(internalNokkelStatusoppdatering)
                    countSuccessfulEventForProducer(event.namespaceAppName)
                } catch (nne: NokkelNullException) {
                    countNokkelWasNull()
                    log.warn("StatusoppdateringInput-eventet manglet nøkkel. Topic: ${event.topic()}, Partition: ${event.partition()}, Offset: ${event.offset()}", nne)
                } catch (fve: FieldValidationException) {
                    countFailedEventForProducer(event.namespaceAppName)
                    val feilrespons = FeilresponsTransformer.createFeilresponsFromNokkel(event.key(), fve, Eventtype.STATUSOPPDATERING)
                    problematicEvents.add(feilrespons)
                    log.warn("Validering av StatusoppdateringInput-event fra Kafka feilet, fullfører batch-en før vi skriver til feilrespons-topic.", fve)
                } catch (cce: ClassCastException) {
                    countFailedEventForProducer(event.namespaceAppName)
                    val funnetType = event.javaClass.name
                    val eventId = event.key().getEventId()
                    val feilrespons = FeilresponsTransformer.createFeilresponsFromNokkel(event.key(), cce, Eventtype.STATUSOPPDATERING)
                    problematicEvents.add(feilrespons)
                    log.warn("Feil eventtype funnet på StatusoppdateringInput-topic. Fant et event av typen $funnetType. Eventet blir forkastet. EventId: $eventId, produsent: ${event.namespaceAppName}, $cce", cce)
                } catch (e: Exception) {
                    countFailedEventForProducer(event.namespaceAppName)
                    val feilrespons = FeilresponsTransformer.createFeilresponsFromNokkel(event.key(), e, Eventtype.STATUSOPPDATERING)
                    problematicEvents.add(feilrespons)
                    log.warn("Transformasjon av StatusoppdateringInput-event fra Kafka feilet, fullfører batch-en før vi skriver til feilrespons-topic.", e)
                }
            }
